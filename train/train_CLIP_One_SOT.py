...
def parse_args_and_logger():
    ...
    #data parameter
    parser.add_argument('--text_dir',type=str,default='./datasets/ShapeNet/feature/train/text', help='directory of text features')
    parser.add_argument('--img_dir',type=str,default='./datasets/ShapeNet/feature/train/image', help='directory of image features')
    ...
    parser.add_argument('--num_layers', type=int, default=9, help='number of hidden layers before output')
    ...
    # train parameter
    parser.add_argument('--batch_size', type=int, default=1200,help='size of the batches')#720*4999
    parser.add_argument('--total_iters', type=int, default=600, help='number of iterations of training')
    
    parser.add_argument('--lambda_cvx', type=float, default=0, help='Weight for positive weight constraints')#1e-4
    parser.add_argument('--lambda_fenchel_eq', type=float, default=0.0, help='Weight for making sure that fenchel equality holds for f')#1e-8
    ...
    parser.add_argument('--lambda_fidelity', type=float, default=20, help='Weight for making sure that OT map')
    ...
    parser.add_argument('--pretrained_dir', type=str, default='./results/One_SOT/', help='Folder for pretrained models')
    parser.add_argument('--test_dir',type=str,default='./datasets/ShapeNet/feature/test', help='directory of test dataset')
    parser.add_argument('--save_dir', type=str, default='./results/One_SOT/', help='Folder for storing obj mesh')
    ...
    parser.add_argument('--sot', type=bool, default=True, help='sot loss or not')
...
def define_network_optimizer(args):
    ...
    logging.info("Created and initialized the convex neural networks 'f'")
    ...
    if args.optimizer == 'Adam':
        ...
        optimizer_f = optim.Adam(convex_f.parameters(), lr=args.lr)

    return convex_f, optimizer_f, f_positive_params


def train_one_epoch(args,data_loader,convex_f, optimizer_f, f_pos_params):
    #convex_f.train()

    running_w_2_loss = 0.0
    running_f_OT_loss = 0.0
    running_f_cvx_loss = 0.0
    for i, (text_feat,img_feat) in enumerate(data_loader):
        ...
        w_2_loss, f_OT_loss, f_cvx_loss = train(args,text_feat,img_feat, 
            convex_f, optimizer_f, f_pos_params,)
        running_w_2_loss += w_2_loss
        running_f_OT_loss += f_OT_loss
        running_f_cvx_loss += f_cvx_loss


    denom = len(data_loader)*args.batch_size
    w_2_loss = running_w_2_loss/denom
    f_OT_loss = running_f_OT_loss/denom
    f_cvx_loss = running_f_cvx_loss/denom

    return w_2_loss, f_OT_loss, f_cvx_loss



def c_transform(X, Y, fx):
    #print('time shape : ',torch.matmul(X_time,Y_time.t()).shape)
    cost_all = torch.log(2-torch.matmul(X,Y.t()))
    #print('cost_all shape : ',cost_all.shape)
    res,_ = torch.min(cost_all- fx,dim=0)
        
    return res

def cal_f_loss(args, f_model, X, Y, eps=1e-9):
    fx = f_model.forward(X).squeeze()

    c_res = c_transform(X, Y, fx)
    f_loss = -torch.mean(fx + c_res)
    ...
    if args.sot:
        ...
        loss = 0
    if args.lambda_fidelity > 0:
        grad_fx = torch.autograd.grad(fx.sum(),X, create_graph=True)[0]
        denom = 1- torch.sum(X*grad_fx,dim=-1,keepdim=True)
        T_X_to_Y = X - grad_fx/(denom+eps)
        norm = torch.sqrt(torch.sum(T_X_to_Y*T_X_to_Y,dim=-1,keepdim=True))
        #print('norm.max=',norm.abs().max())
        T_X_to_Y = T_X_to_Y/norm
      
        constraint_loss = args.lambda_fidelity*(T_X_to_Y - Y).pow(2).sum(dim=-1).mean()
    ...
    if args.lambda_fenchel_ineq > 0:
        ...
        ineq_reg = args.lambda_fenchel_ineq*(fx+c_res-cost).pow(2).mean()
        loss += ineq_reg
    ...
    if args.lambda_fenchel_eq > 0:
        ...
        c_res1 = c_transform(X, T_X_to_Y, fx)
        eq_reg = args.lambda_fenchel_eq*(fx+c_res1-cost).pow(2).mean()
        loss += eq_reg
        #print('f_eq_reg=',eq_reg.item())

        
    return loss, f_loss,c_res

def compute_W2(f_model,X, Y):
    fx = f_model.forward(X).squeeze()
    c_res = c_transform(X, Y, fx)
    W2 = torch.mean(fx + c_res)

    return W2

def train(args,text_feat,img_feat, convex_f, optimizer_f, f_pos_params):
    ...
    f_cvx_loss = 0
    loss, f_loss,c_res = cal_f_loss(args, convex_f, x, y)
    ...
    if args.lambda_cvx > 0:
        f_cvx_loss /= args.lambda_cvx

    fx = convex_f.forward(x).squeeze()
    W2 = torch.mean(fx + c_res)

    return W2.item(), f_OT_loss, f_cvx_loss
...
if __name__ == '__main__':
    ...
    pretrained_dir = './results/Train_SOT/'
    convex_f,optimizer_f,f_pos_params = define_network_optimizer(args)
    ...
    args.save_dir = args.save_dir + "cvx_{0}/fide_{1}/lr_{2}/act_{3}/quad_{4}/layer_{5}/try_{6}".format(args.lambda_cvx,
									args.lambda_fidelity,args.lr,args.activation,args.full_quadratic,args.num_layers,args.trys)
    ...
    for iteration in range(1, args.total_iters + 1):
        ...
        w_2_loss, f_OT_loss, f_cvx_loss = train_one_epoch(args,train_loader,
                    convex_f, optimizer_f, f_pos_params)

        if iteration % args.log_interval == 0:
            print('Iteration: {} [{}/{} ({:.0f}%)] OT_loss: {:.4f} cvx_Loss: {:.4f}  W_2_loss: {:.4f} '.format(
                iteration, iteration, args.total_iters, 100. * iteration / args.total_iters, 
                f_OT_loss, f_cvx_loss, w_2_loss))
        ...
        if iteration % 10 == 0:
            ...
            for k, latent in enumerate(latents):
                ...
                #print('text[k] = ',text[k])
                basename = text[k][:240] + f'_text_{iteration}.obj'
