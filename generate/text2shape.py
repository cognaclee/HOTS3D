...
def parse_args():
    ...
    # train parameter
    parser.add_argument('--save_dir', type=str, default='./results/test-time', help='Folder for storing obj mesh')#sot-mesh
...
if __name__ == '__main__':
    ...
    text_file = '/home/lwh/code/shap-e/datasets/ShapeNet/feature/test/text.txt'
    text_cont = []
    ...
    text_file = '/home/lwh/code/shap-e/datasets/ShapeNet/feature/test/name.txt'
    img_list = []
    with open(text_file, 'r') as f:
        for line in f.readlines():
            line = line.strip('\n') 
    ...
    text_file = '/home/lwh/code/shap-e/datasets/ShapeNet/feature/test/obj.txt'
    name_list = []
    with open(text_file, 'r') as f:
        for line in f.readlines():
            line = line.strip('\n') 
    ...
    save_dir = '/data/result/OT/mesh_new/'
    ...
    for cnts in range(len(text_cont)): 
        ...
        if prob<0.8:
            #print('mesh_dir=', obj_dir+name_list[cnts])
            mesh_file = os.path.join(obj_dir+name_list[cnts],"model.obj")
            #print('mesh_file=', mesh_file)
            shutil.copy2(mesh_file, save_dir)
            #target_file = os.path.join(save_dir, "new_" + file_name)
            target_file = os.path.join(save_dir, basename)
            os.rename(os.path.join(save_dir, "model.obj"), target_file)
        else:
            ...
            if prob2>0.5:
                ...
                imgname = img_list[cnts]
                img = load_image(imgname)
                model_kwargs = dict(images=[img])
                latents = generate_latent(model2, diffusion, img_guidance_scale, model_kwargs)
                print('image')
