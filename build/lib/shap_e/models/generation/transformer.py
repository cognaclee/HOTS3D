...
class CLIPImagePointDiffusionTransformer(PointDiffusionTransformer):
    def __init__(
        ...
        **kwargs,
        ...
        self.clip_feat = None
        print('******** self.clip.clip_name=',self.clip.model.clip_name)
    ...
    def forward(
        ...
        t_embed = self.time_embed(timestep_embedding(t, self.backbone.width))
        clip_out = None
        if txt2img_clip is not None:
            clip_out = txt2img_clip
        elif first:
            self.clip_feat = self.clip(batch_size=len(x), images=images, texts=texts, embeddings=embeddings)
            #print('clip_text.shape=',self.clip_feat.shape)
            clip_out = self.clip_feat
        if clip_out is None:
            self.clip_feat = self.clip(batch_size=len(x), images=images, texts=texts, embeddings=embeddings)
            #print('clip_text.shape=',self.clip_feat.shape)
            clip_out = self.clip_feat
...
class CLIPImageGridPointDiffusionTransformer(PointDiffusionTransformer):
    ...
    def forward(
        ...
        #assert images is None or txt2img_clip is None, "cannot specify both images and embeddings"
        assert x.shape[-1] == self.n_ctx
        ...
        if images is not None:
            ...
            clip_out = txt2img_clip
        #print('clip_out.shape=',clip_out.shape)
