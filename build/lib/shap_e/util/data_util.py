...
def load_or_create_multiview(
    *,
    ...
    if cache_dir is not None:
        #print('path=',path)
        #print('bf.basename(path)=',bf.basename(path))
        basename = bf.basename(path)[:100]
        print('basename0=',basename)
        if extract_material:
            cache_path = bf.join(cache_dir, f"mv_{basename}_mat_{num_views}.zip")
        else:
            cache_path = bf.join(cache_dir, f"mv_{basename}_{light_mode}_{num_views}.zip")
        if bf.exists(cache_path):
