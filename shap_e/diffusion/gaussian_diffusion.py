...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        self, model, x, t, clip_denoised=False, denoised_fn=None, model_kwargs=None,first=False
    ):
        """
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output = model(x, t, first, **model_kwargs)
        #print(model_output.shape)
        if isinstance(model_output, tuple):
            model_output, extra = model_output
        else:
            extra = None
            #print('model_output is no tuple=')
